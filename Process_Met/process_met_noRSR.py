...
class ProcessMet(object):
    ...
    def _incidence(self, jday, hr, mins, secs):
        ...
        std = 15 * abs(self.zone)  # standard meridian
    ...
    def _retrieve_grib_file(self, date, gribfile):
        """
        ...
        self.ftp.cwd(str(date.year)) # Year directory
        self.ftp.cwd(str("%03d" % date.dayofyear)) # Day directory
    ...
    def _extract_from_nasa(self, date):
        ...
        FTPURL='hydro1.sci.gsfc.nasa.gov'
        ...
        if not self.ftp:
            ...
            self.ftp = ftplib.FTP(FTPURL, 'anonymous', 'user', timeout=1200)
            #ftp.debug(1)
        ...
        if not local_copy:
            self._retrieve_grib_file(date, gribfile)
    ...
    def _extract_from_metdata(self, date):
        ...
        if met_global < 0:
            met_global = 0.0
        #met_direct = self.met_total.loc[date].Direct_Avg
        # Assuming global is equal to the direct and no diffuse for now
        met_direct = met_global
        #met_diffuse =  self.met_total.loc[date].Diffuse_Avg
        met_diffuse = 0
    ...
    def read_met_files(self, excel_met, excel_met_rsr, to_csv=False):
        """
        ...
        if type(metxls.index) != pd.tseries.index.DatetimeIndex:
        ...
        if excel_met_rsr != None: 
            ...
            if type(metxls_rsr.index) != pd.tseries.index.DatetimeIndex:
    ...
    def process_data(self, startRange=None, endRange=None, date_range=None,
        ...
        if self.met_total is None and metdf is None:
        ...
        if self.met_total is None and metdf:
        ...
        if startRange is None:
            ...
            for i in range(len(self.met_total)):
                if onHour:
                    ...
                    if self.met_total.index[i].minute == 0 and \
        ...
        for date in date_range:
            try:
                ...
                if self.pull_grib is True:
                    ...
                    except Exception, e: # Most likely a timeout
                        print e
                        print "Closing ftp connection for reopening"
                        self.ftp.close()
                        self.ftp = None
                        sleep(2)
                        self._extract_from_nasa(date)
