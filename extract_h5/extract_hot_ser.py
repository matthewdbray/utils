...
if len(sys.argv) != 5: 
    print "Usage: extract_hot_ser.py <sim_name> <hr_to_extract> <total_nodes> <total_elements>"

sim_name = sys.argv[1]
hr_to_extract = sys.argv[2]
...
#def local_hot_list(filename, hr_to_extract, rank):

proc_hot_list = numpy.zeros(total_nodes*2)
proc_hot_list = proc_hot_list.reshape(total_nodes,2)
...
for filename in files:
   ...
   temp_day = numpy.array(temp[hr_to_extract])
   press_head = data['Pressure_Head']
   press_day = numpy.array(press_head[hr_to_extract])
   geo = f1['Geometry']
   ...
   orig_nodes = numpy.array(nodes['0'])
    ...
    #    it.iternext()
   for i in xrange(len(orig_nodes)):
       proc_hot_list[orig_nodes[i]] = [press_day[i], temp_day[i]]
...
#a = tuple(local_hot_list(name, hr_to_extract, rank) for name in local_files)
...
for i in xrange(len(proc_hot_list)):
    hotlist[i] = [proc_hot_list[i][0], proc_hot_list[i][1]]
...
path = os.getcwd()
path += '/'
hotstart_file = path + sim_name + '_' + hr_to_extract + '.hot'
